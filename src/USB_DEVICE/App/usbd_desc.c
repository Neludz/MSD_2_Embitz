...
{
  UNUSED(speed);
  *length = sizeof(USBD_FS_StringManufacturer);
  //USBD_GetString((uint8_t *)USBD_MANUFACTURER_STRING, USBD_StrDesc, length);

  return (uint8_t *) USBD_FS_StringManufacturer;//USBD_StrDesc;
}
...
uint8_t * USBD_FS_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
    *length = sizeof(Composite_StringSerial);
//  UNUSED(speed);
...
//  /* USER CODE END USBD_FS_SerialStrDescriptor */
  return (uint8_t *)  Composite_StringSerial;//USBD_StringSerial;
}
...
//    USBD_GetString((uint8_t *)USBD_CONFIGURATION_STRING_FS, USBD_StrDesc, length);
//  }
  return   (uint8_t *) Composite_StringSerial;//USBD_StrDesc;
}
